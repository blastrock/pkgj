...
std::string get_image_url(DbItem *item) 
{
    ...
    return fmt::format("https://store.playstation.com/store/api/chihiro/"
        "00_09_000/container/{}/{}/19/{}/{}/image?w=248&h=248",
        country_abbv, language, item->content, pkgi_time_msec());
}
...
void ImageFetcher::do_request()
{
    try
    {
        ...
        const auto image = download_data(_http.get(), _url);
        {
            ...
            else if ((_texture = vita2d_load_JPEG_buffer(image->data(), image->size())) != nullptr)
